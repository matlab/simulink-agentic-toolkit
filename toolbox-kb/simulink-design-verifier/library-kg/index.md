# Library Reuse Index

## Priority

1. Custom library blocks (highest priority)
2. Toolbox KB blocks

## Policy

- Always use custom library blocks when available.
- Never fall back to built-in primitives if the same block exists in a declared library.
- Only use built-in blocks when NO equivalent exists in any declared library after searching this index.
- Do not invent custom block names.
- If uncertain, inspect the relevant category page or ask the user.

## Libraries

- Simulink Design Verifier

Common blocks: [common.md](common.md) (4 of 11 blocks)

## Categories

- [Verification utils](verification-utils.md) — 4 blocks; Verification infrastructure and utilities
- [Objectives constraints](objectives-constraints.md) — 4 blocks; Formal verification objectives and input constraints
- [Temporal operators](temporal-operators.md) — 3 blocks; Temporal logic operators for time-based properties
